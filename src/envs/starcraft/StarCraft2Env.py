...
class StarCraft2Env(MultiAgentEnv):
    ...
    def reward_battle(self):
        ...
        Returns accumulative(累计) hit/shield point damage(造成命中/护盾伤害) dealt to the enemy
        + reward_death_value(击杀) per enemy unit killed, and, in case
        self.reward_only_positive == False, - (damage(受到命中/护盾伤害) dealt to ally units
        + reward_death_value(阵亡) per ally unit killed) * self.reward_negative_scale
        """
    ...
    def save_replay(self):
        ...
        prefix += f"_{self.get_stats()['win_rate']}"
    ...
    def kill_units_by_id(self, killed_ally_ids):
        """Kill units on the map by unit_ids."""
        units_alive = [
            unit.tag for unit_id, unit in self.agents.items() if unit.health > 0 and unit_id in killed_ally_ids
        ]
        debug_command = [
    ...
    def init_units(self):
        ...
        while True:
            ...
            """My Code"""
            # random kill
            # self.kill_units_by_id([np.random.randint(low=0, high=len(ally_units_sorted))])
            # random max contribution agent (id)
            # self.kill_units_by_id([9])    # Max attacks
            # self.kill_units_by_id([7])    # Max survivals
            # random min contribution agent (id)
            # self.kill_units_by_id([12])   # Min attacks
            # self.kill_units_by_id([12])   # Min survivals
